...
函数 OUT 的功能是，在送出一个单词的内部表示之后，返回到调用该词法分析程序的那个程序。C:\Users\tang\Desktop*/
{
    ofstream fout("C:\\Users\\tang\\Desktop\\result.txt", fstream::app);
    char temp = ' ';
    if (a == 8 || a == 9)
    {
...
int main()
{
    ...
    char strr[50];//输入的表达式
    int i = 0;
    ofstream out1("C:\\Users\\tang\\Desktop\\test.txt");
    out1.close();
    ofstream out("C:\\Users\\tang\\Desktop\\test.txt", ios::out);
    while (true)
    {
        cin >> strr[i];
        out << strr[i];
    ...
    FILE* fp1;
    FILE* fp2;
    fp2 = fopen("C:\\Users\\tang\\Desktop\\result.txt", "w");
    fclose(fp2);
    fp1 = fopen("C:\\Users\\tang\\Desktop\\test.txt", "r");
    if (!fp1)
    {
        printf("打开文件失败！");
    }
    ...
    cout << endl;
    cout << "词法分析结果为：" << endl;
    ifstream myfile2("C:\\Users\\tang\\Desktop\\result.txt", ios::in);
    if (!myfile2.is_open())
    {
        cout << "未成功打开文件" << endl;
    }
